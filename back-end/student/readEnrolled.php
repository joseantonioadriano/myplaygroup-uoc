...
$queryStudentRead->setSQL('select s.* from MPG_STUDENTS s where s.idSchool= '.$_GET['idSchool'].' and s.id in (select e.idStudent from MPG_ENROLLMENTS e, MPG_COURSES c where e.idCourse = c.id and e.idSchool= '.$_GET['idSchool'].' and c.active = 1)');
